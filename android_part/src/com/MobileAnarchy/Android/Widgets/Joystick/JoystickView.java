...
public class JoystickView extends View {
    ...
    // максимальная координата. джойстик возвращает координаты в диапазоне от 0 до COORDINATES_COEFF, для получения диапазона
    // от 0 до 1 следует разделить на эту константу
    public static final int COORDINATE_MAX = 10;

    // Значение для смещения координат чтобы можно было летать без маленьких случайных отклонений в побочном направлении
    public static final int COORDINATE_SHIFT = 3;

    public static final float MOVEMENT_RESOLUTION = 10.0f;

    public static final float CLICK_THRESHOLD = 0.4f;


    private final boolean D = false; // Debug info
	...
	private void initJoystickView() {
		...
		setMovementRange(COORDINATE_MAX);
		setMoveResolution(MOVEMENT_RESOLUTION);
		setClickThreshold(CLICK_THRESHOLD);
...
//		Log.d(TAG, String.format("onDraw(%.1f,%.1f)\n\n", handleX, handleY));
	...
	private void constrainCircle() {
		...
		if ( radial > movementRadius ) {
			touchX = (int)Math.round((diffX / radial) * movementRadius);
			touchY = (int)Math.round((diffY / radial) * movementRadius);
		}
	}
...
//        	Log.d(TAG, String.format("ACTION_MOVE: (%03.0f, %03.0f) => (%03.0f, %03.0f)", x, y, touchX, touchY));
	...
	private void reportOnMoved() {
		...
		if (moveListener != null) {
			boolean rx = Math.abs(touchX - reportX) >= getMoveResolution();
			boolean ry = Math.abs(touchY - reportY) >= getMoveResolution();
			if (rx || ry) {
				this.reportX = touchX;
				this.reportY = touchY;
				
//				Log.d(TAG, String.format("moveListener.OnMoved(%d,%d)", (int)userX, (int)userY));
				moveListener.OnMoved(getUserX(), getUserY());
			}
		}
	}
	...
	private void calcUserCoordinates() {
		...
		if ( userCoordinateSystem == COORDINATE_CARTESIAN ) {
			...
			setUserX(cartY + cartX / 4);
			setUserY(cartY - cartX / 4);
			
			if ( getUserX() < -movementRange )
				setUserX((int)-movementRange);
			if ( getUserX() > movementRange )
				setUserX((int)movementRange);

			if ( getUserY() < -movementRange )
				setUserY((int)-movementRange);
			if ( getUserY() > movementRange )
				setUserY((int)movementRange);
		}
...
//				Log.d(TAG, "click");
    ...
    private int shiftCoordinate(int coord) {
      //координаты смещаем на COORDINATE_SHIFT, для более плавных полетов в одном направлении (без случайных смещений в побочном направлении)
      int coordShifted = ((coord < 0) ? -1 : 1) *  Math.max(Math.abs(coord) - COORDINATE_SHIFT, 0);
      return Math.min(coordShifted * COORDINATE_MAX / (COORDINATE_MAX - COORDINATE_SHIFT), COORDINATE_MAX);
    }
